...
<InstallTabs command="shadcn add @fancy/marquee-along-svg-path" npx />
...
SVG paths are defined with fixed coordinates, which can make scaling tricky. This component supports a **Scale Container** technique to make the marquee responsive.

### Scale Container Technique
...
<CodeSnippet title="Enabling responsive mode">
```jsx
<MarqueeAlongSvgPath
  path={path}
...
<Table>
  ...
  <TableBody>
    ...
    <TableRow>
      ...
      <TableCell>`boolean`</TableCell>
      <TableCell>`false`</TableCell>
      <TableCell>Whether to enable responsive scaling and centering</TableCell>
    </TableRow>
  </TableBody>
</Table>
